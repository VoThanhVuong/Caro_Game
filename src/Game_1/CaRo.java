...
public class CaRo extends JFrame implements ActionListener {
    ...
    List<String> stepSystem = new ArrayList<>();
    List<NextStep> stepPlayer = new ArrayList<>();
    ...
    public void system(int i, int j) {
        if (CaRoDAO.checkWin(i, j, b)) {
            ...
            NextStep nextStep = CaRoDAO.defectStep(i, j, b, stepSystem);
            int mark = nextStep.getMark();
            NextStep s = null;
            for (NextStep p : stepPlayer) {
                if (mark > p.getMark()) {
                    mark = p.getMark();
                    s = p;
                }
            ...
            if (mark != nextStep.getMark()) {
                System.out.println(s.getPoint1() + ":" + mark);
                int k = s.getPoint1().indexOf(32);
                i = Integer.parseInt(s.getPoint1().substring(0, k));
                j = Integer.parseInt(s.getPoint1().substring(k + 1, s.getPoint1().length()));
                if (tick[i][j]) {
                    addPoint(i, j);
                }
                if (CaRoDAO.checkWin(i, j, b)) {
                ...
                if (nextStep.getPoint2() == "") {
                    boolean check1 = true;
                    for (String point : stepSystem) {
                        if (point.equals(nextStep.getPoint1())) {
                    ...
                    if (check1) {
                        stepSystem.add(nextStep.getPoint1());
                    }
                    for (NextStep point : stepPlayer) {
                        if (point.getPoint1().equals(nextStep.getPoint1())) {
                            stepPlayer.remove(point);
                            break;
                        }
                    }
                    int k = nextStep.getPoint1().indexOf(32);
                    i = Integer.parseInt(nextStep.getPoint1().substring(0, k));
                    j = Integer.parseInt(nextStep.getPoint1().substring(k + 1, nextStep.getPoint1().length()));
                    if (tick[i][j]) {
                        addPoint(i, j);
                    }
                    if (CaRoDAO.checkWin(i, j, b)) {
                    ...
                    boolean check1 = true;
                    for (String point : stepSystem) {
                        if (point.equals(nextStep.getPoint2())) {
                            stepSystem.remove(point);
                            check1 = false;
                            break;
                        }
                    ...
                    if (check1) {
                        stepSystem.add(nextStep.getPoint2());
                    }
                    int k = nextStep.getPoint2().indexOf(32);
                    i = Integer.parseInt(nextStep.getPoint2().substring(0, k));
                    j = Integer.parseInt(nextStep.getPoint2().substring(k + 1, nextStep.getPoint2().length()));
                    if (tick[i][j]) {
                        addPoint(i, j);
                    }
                    if (CaRoDAO.checkWin(i, j, b)) {
                        ...
                        k = nextStep.getPoint1().indexOf(32);
                        i = Integer.parseInt(nextStep.getPoint1().substring(0, k));
                        j = Integer.parseInt(nextStep.getPoint1().substring(k + 1, nextStep.getPoint1().length()));
                        nextStep.setMark(CaRoDAO.mark(i, j, b, "X"));
                        stepPlayer.add(nextStep);
                    }
                }
            }
        }
